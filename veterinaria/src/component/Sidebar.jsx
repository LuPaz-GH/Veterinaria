...
const Sidebar = () => {
  ...
  // Turnos (solo Admin, Veterinario y Recepcionista)
  if (esVeterinario || esAdmin || esRecepcionista) {
  ...
  // Medicamentos → Oculto temporalmente
  // if (esRecepcionista || esAdmin) {
  //   menuItems.push({ name: 'Medicamentos', path: '/medicamentos', icon: faPills });
  // }

  // Caja
  if (esRecepcionista || esAdmin) {
    menuItems.push({ name: 'Caja', path: '/caja', icon: faCashRegister });
  }
  ...
  return (
    <div className="d-flex flex-column flex-shrink-0 p-3 text-white shadow-lg" 
      ...
      <button className="btn btn-outline-light rounded-pill d-flex align-items-center gap-3 py-3 mb-3" 
