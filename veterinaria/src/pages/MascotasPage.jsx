...
const MascotasPage = () => {
  ...
  const cargarDatos = async () => {
    try {
      const [resM, resD] = await Promise.all([
        ...
        fetch('http://localhost:3001/api/duenos')
      ]);
      setMascotas(await resM.json());
      setDuenos(await resD.json());
  ...
  const handleGuardarMascota = async (e) => {
    e.preventDefault();
    const url = datosEdicion ? `http://localhost:3001/api/mascotas/${datosEdicion.id}` : 'http://localhost:3001/api/mascotas';
    await fetch(url, {
  ...
  const handleGuardarQuickDueno = async (e) => {
    e.preventDefault();
    const res = await fetch('http://localhost:3001/api/duenos', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify(quickDueno)
    });
    const resData = await res.json();
    if (resData.success) {
      await cargarDatos(); 
      setFormData({ ...formData, dueno_id: resData.id }); 
      setShowQuickDueno(false);
    }
  };

  const confirmarEliminar = async () => {
    await fetch(`http://localhost:3001/api/mascotas/${idEliminar}`, { method: 'DELETE' });
    ...
    <div className="container-fluid p-4">
      ...
      <div className="row justify-content-center mb-5">
        <div className="col-md-6">
          <div className="input-group rounded-pill bg-white overflow-hidden shadow-sm border-0">
            <span className="input-group-text bg-white border-0 ps-4"><FontAwesomeIcon icon={faSearch} className="text-muted"/></span>
            <input type="text" className="form-control border-0 py-3" placeholder="Buscar..." value={busqueda} onChange={(e) => setBusqueda(e.target.value)} />
            {busqueda && <button className="btn bg-white border-0 text-muted pe-4" onClick={() => setBusqueda('')}><FontAwesomeIcon icon={faTimes} /></button>}
          </div>
        </div>
      </div>
      ...
      <div className="row g-4">
        {mascotasFiltradas.map(m => (
          <div className="col-md-3" key={m.id}>
            <div 
              className="card border-0 shadow-sm p-4 rounded-4 h-100 text-center bg-white transition-card" 
